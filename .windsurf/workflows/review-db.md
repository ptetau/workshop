...
2. **Schema** (`internal/adapters/storage/db.go`) — if tables or columns changed:
   - Tables use `CREATE TABLE IF NOT EXISTS` (idempotent on startup)
...
3. **Store structure** (`internal/adapters/storage/*/`) — if stores changed:
...
4. **Write queries** — if INSERT/UPDATE/DELETE queries changed:
...
5. **Read queries** — if SELECT queries or projections changed:
...
6. **Scan helpers** — if scan functions changed:
...
7. **Configuration** — if DSN or connection setup changed:
...
8. **Operations** — if backup or DB file handling changed:
